...
export default function PDFToExcelPage() {
    ...
    // States: "upload", "processing", "success", "error"
    ...
    function handleFileSelected(f) {
        ...
        setFile(f);
        setSourceUrl("");
        handleProcess(f, null);
    }

    async function handleProcess(uploadFile, urlStr) {
        ...
        try {
            ...
            // Slower progress bar to account for heavy pdftable parsing
            ...
            const res = await fetch(`${API_BASE}/api/convert/pdf-to-excel`, {
                method: "POST",
                body: formData,
            });
            ...
            setProgress(0);
            setErrorMsg(err.message || "Failed to convert PDF to Excel.");
            setStep("error");
        }
    }
    ...
    return (
        <>
            <Head>
                <title>Convert PDF to Excel - AllTools</title>
                <meta name="description" content="Extract tables and data from PDF to structured Excel (.xlsx) spreadsheets instantly." />
            </Head>
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <h1 className="text-3xl font-bold mb-2">PDF to Excel Converter</h1>
                <p className="text-gray-500 mb-8 max-w-2xl">
                    Instantly extract layout tables, rows, and columns from your PDF document and download as a formatted Excel spreadsheet (.xlsx).
                </p>

                {/* Upload Step */}
                {step === "upload" && (
                    <div
                        className={`rounded-2xl border-2 p-10 transition-all bg-white relative ${dragOver ? "border-green-500 bg-green-50" : "border-gray-200"}`}
                    ...
                    >
                        <div className="flex flex-wrap gap-2 mb-8 justify-center">
                            {[{ id: "device", icon: "💻", label: "My Device" }, { id: "url", icon: "🔗", label: "Paste URL" }, { id: "drive", icon: "🟢", label: "Google Drive" }, { id: "dropbox", icon: "🔵", label: "Dropbox" }].map(m => (
                                <button key={m.id} onClick={() => setUploadMethod(m.id)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border text-sm font-medium transition-all ${uploadMethod === m.id ? "bg-green-600 text-white border-green-600 shadow" : "bg-white text-gray-600 border-gray-300 hover:border-green-400"}`}>
                        ...
                        {uploadMethod === "device" && (
                            <div onClick={() => fileInputRef.current?.click()} className="text-center cursor-pointer py-10 border-2 border-dashed rounded-xl border-gray-300 hover:border-green-400 hover:bg-green-50/50">
                                ...
                                <div className="mt-6 flex items-center justify-center gap-3 text-sm font-medium text-gray-400">
                                    <span className="px-3 py-1 bg-gray-100 rounded-full">Contains Tables</span>
                                    <span className="px-3 py-1 bg-gray-100 rounded-full">PDF format</span>
                                </div>
                        ...
                        {uploadMethod === "url" && (
                            <div className="max-w-xl mx-auto space-y-4 py-8">
                                <input type="url" value={sourceUrl} onChange={e => setSourceUrl(e.target.value)} placeholder="https://example.com/document.pdf" className="w-full border-2 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-400" />
                                <button onClick={() => handleUrlSelected(sourceUrl)} disabled={!sourceUrl.trim()} className="w-full py-3 bg-green-600 text-white rounded-xl font-semibold disabled:opacity-50">Continue</button>
                            </div>
                        )}

                        {uploadMethod === "drive" && (
                            <div className="flex justify-center py-8">
                                <button onClick={handleGoogleDrivePick} className="px-6 py-3 bg-white border-2 rounded-xl font-medium text-gray-700 hover:border-green-500 shadow-sm flex items-center gap-3">
                        ...
                        {uploadMethod === "dropbox" && (
                            <div className="flex justify-center py-8">
                                <button onClick={handleDropboxPick} className="px-6 py-3 bg-white border-2 rounded-xl font-medium text-gray-700 hover:border-green-500 shadow-sm flex items-center gap-3">
                ...
                {step === "processing" && (
                    <div className="w-full max-w-md mx-auto mt-16 p-8 bg-white border rounded-2xl shadow-xl flex flex-col items-center animate-fade-in">
                        ...
                        <h3 className="text-lg font-bold text-gray-800 mb-4">Extracting Tables...</h3>
                        <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden mb-2">
                            <div className="h-full bg-gradient-to-r from-green-500 to-emerald-600 transition-all duration-300 ease-out" style={{ width: `${progress}%` }}></div>
                        </div>
                        <div className="flex justify-between w-full text-xs font-medium text-gray-500">
                            <span>Parsing PDF structures</span>
                            <span>{progress}%</span>
                        </div>
                ...
                {step === "success" && (
                    <div className="w-full max-w-lg mx-auto mt-10 p-8 bg-white border border-green-200 rounded-3xl shadow-xl flex flex-col items-center animate-fade-in">
                        ...
                        <button onClick={() => downloadFile(downloadUrl, "AllTools_Extracted.xlsx")} className="w-full py-4 bg-green-600 hover:bg-green-700 text-white text-lg rounded-xl font-bold shadow transition flex items-center justify-center gap-2 mb-4 cursor-pointer">
                            Download Excel File (.xlsx)
                        </button>

                        <div className="flex gap-4 w-full">
                            <button onClick={() => { setStep("upload"); setFile(null); }} className="flex-1 py-3 text-gray-600 font-semibold rounded-xl hover:bg-gray-100 transition border">Convert Another</button>
                        </div>
                    </div>
                )}
