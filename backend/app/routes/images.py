...
def png_to_jpg(
    ...
    """Convert PNG to JPG"""
    quality = clean_param(quality, 95, int)
...
def jpg_to_webp(
    ...
    quality = clean_param(quality, 85, int)
    if file and file.filename and not file.filename.lower().endswith(('.jpg', '.jpeg')):
        raise HTTPException(status_code=400, detail="Only JPG/JPEG files allowed")
...
def image_compress(
    ...
    """
    quality = clean_param(quality, 75, int)
    max_width = clean_param(max_width, None, int)
    max_height = clean_param(max_height, None, int)
    format = clean_param(format, "original", str)
...
def remove_background(
    ...
    """
    bgcolor = clean_param(bgcolor, None, str)
    ...
    try:
        ...
        if bgcolor and isinstance(bgcolor, str) and bgcolor.strip():
...
def image_resize(
    ...
    format: Optional[str] = Form("original")
):
    """
    Resize image by exact width/height or by percentage.
    """
    width = clean_param(width, None, int)
    height = clean_param(height, None, int)
    percentage = clean_param(percentage, None, int)
    format = clean_param(format, "original", str)
